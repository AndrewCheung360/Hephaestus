...
const DEFAULT_VEO_MODEL = self.HephApiConfig.DEFAULT_VEO_MODEL;
...
async function generateVideo({
  ...
  try {
    ...
    const hint =
      'Cannot reach Hephaestus local server. From the repo, run: cd server && npm start (see server/.env.example).';
  ...
  while (true) {
    ...
    const pollPath = `/gemini/v1beta/${operationName}`;
