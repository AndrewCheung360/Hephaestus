...
// Default to Veo 3 GA. Override via chrome.storage.local.veoModel from the side panel.
// Other valid IDs: veo-3.0-fast-generate-001, veo-3.1-generate-preview,
// veo-3.1-fast-generate-preview, veo-3.1-lite-generate-preview, veo-2.0-generate-001.
const DEFAULT_VEO_MODEL = 'veo-3.0-generate-001';
const POLL_INTERVAL_MS = 5000;
const POLL_TIMEOUT_MS = 6 * 60 * 1000; // 6 minutes

// Veo 3.x has a fixed 8-second duration and ignores (or rejects) durationSeconds.
// Only Veo 2 accepts a configurable duration.
function modelAcceptsDuration(modelId) {
  return /^veo-2\./.test(modelId);
}
...
async function generateVideo({
  ...
  // The docs send durationSeconds as a string; Veo 3.x ignores/rejects it entirely.
  if (modelAcceptsDuration(model)) {
    parameters.durationSeconds = String(durationSec);
  }
  const submitBody = {
    instances: [{ prompt }],
    parameters
  ...
  if (!submitResp.ok) {
    ...
    throw new Error(`Veo submit failed ${submitResp.status} for model "${model}": ${errText.slice(0, 300)}`);
  }
